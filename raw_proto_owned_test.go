...
// bipWith returns a valid raw client config on the bip profile with a custom outer protocol number.
func bipWith(proto int) *Config {
	c := validRaw()
	c.RawProfile, c.RawProto = "bip", proto
	return c
}
...
// TestBipRefusesAProtocolNumberAnotherProfileOwns closes the shape that reaches the operator as packet
// loss and nothing else. bip writes NO L4 header, so raw_proto 6 puts ciphertext where a middlebox
...
func TestBipRefusesAProtocolNumberAnotherProfileOwns(t *testing.T) {
	for _, name := range packet.RawProfileNames() {
		n := protoOf(t, name)
		err := bipWith(n).validate()
		if name == "bip" { // its own native number is not a borrowed one
			if err != nil {
				t.Errorf("bip must accept its own protocol number %d, got %v", n, err)
			}
			continue
		}
		if err == nil {
			t.Errorf("bip + raw_proto %d (owned by %q) must be refused: it sends the number with no header", n, name)
			continue
		}
	...
	for _, free := range []int{2, 143, 200, 252, 254} {
		...
		if err := bipWith(free).validate(); err != nil {
			t.Errorf("bip + raw_proto %d owns no header and must be allowed, got %v", free, err)
		}
	}

	// The rule stays bip-only: another profile may not override its number at all, free or not. That is
	// a pre-existing rule, pinned here so the new branch cannot swallow it.
	c := validRaw()
	c.RawProfile, c.RawProto = "tcp", 200
	if err := c.validate(); err == nil {
		t.Error("a non-bip profile must still refuse raw_proto outright — its number is tied to its header")
	}

	// The SPOOF carrier is headerless in exactly the same way and takes the same knob, so it takes the
	// same rule. Left out, the number the operator could not set on bip is still settable one tab away.
	sp := validSpoof()
	sp.RawProto = protoOf(t, "tcp")
	if err := sp.validate(); err == nil {
		t.Error("spoof is bip-like and headerless — it must refuse a borrowed protocol number too")
	}
	sp.RawProto = 200
