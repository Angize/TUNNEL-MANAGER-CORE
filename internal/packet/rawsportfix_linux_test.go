...
func TestAFixedSourcePortReachesTheWireThePathAndTheAntiLeakRule(t *testing.T) {
	...
	for _, isClient := range []bool{true, false} {
		got := rawDropMatches(rawLeak{peer: testDst, profile: "tcp", port: rawServerPort, isClient: isClient})
		if len(got) != 1 {
...
func TestAFixedSourcePortIsDroppedWhereItCannotHold(t *testing.T) {
	...
	for _, isClient := range []bool{true, false} {
		rule := strings.Join(rawDropMatches(rawLeak{peer: testDst, profile: "tcp", port: rawServerPort, isClient: isClient})[0], " ")
