...
// PROACTIVE rotation must now pass over (re-admission is the out-of-band prober's call), and that only
// a failover with nothing healthy left may land on.
...
// endpoint: a FAILOVER with nothing healthy left, which must go somewhere. Its choice is honoured by
...
func TestFailoverLandsOnADueEndpointAndIsNotSticky(t *testing.T) {
	for _, tc := range []struct {
		...
		{"the retry was re-admitted by the prober", func(p *PeerPool) { p.retestResult("10.0.0.2", true) }, "10.0.0.2"},
	} {
