...
func TestUDPServerMultiIP(t *testing.T) {
	...
	cli, err := Dial(a2, cliDev, false, true, psk, cipher, false, 0, 0)
	if err != nil {
		t.Fatalf("Dial: %v", err)
	}
	go srv.Run()
...
func TestTCPServerMultiIP(t *testing.T) {
	...
	cli, err := DialTCP(a2, cliDev, false, true, psk, cipher, false, "")
	if err != nil {
...
func runTunnel(t *testing.T, transport string, obfs bool) {
	...
	if transport == "tcp" {
		addr := freeTCPPort(t)
		srv, err = ListenTCP([]string{addr}, srvDev, obfs, cryptoOn, psk, cipher, false, "")
		if err != nil {
			t.Fatalf("ListenTCP: %v", err)
		}
		cli, err = DialTCP(addr, cliDev, obfs, cryptoOn, psk, cipher, false, "")
		if err != nil {
			t.Fatalf("DialTCP: %v", err)
		}
	} else {
		addr := freeUDPPort(t)
		srv, err = Listen([]string{addr}, srvDev, obfs, cryptoOn, psk, cipher, false, 0, 0)
		if err != nil {
			t.Fatalf("Listen: %v", err)
		}
		cli, err = Dial(addr, cliDev, obfs, cryptoOn, psk, cipher, false, 0, 0)
		if err != nil {
			t.Fatalf("Dial: %v", err)
		}
	}
