...
// PeerPool reports "rotation has stopped, one left" for whichever axis it is -- the detail carries
// dst: or src:. wsPool counted only its IP half and hard-coded the "ip:" prefix, so an operator whose
// fronting SNIs were all burned got no warning at all: the pool is pinned to its last name and the
// panel is silent. The panel already renders the sni: axis; the core never sent it.
func TestBothEdgeAxesReportAStoppedRotation(t *testing.T) {
	t.Run("the sni axis", func(t *testing.T) {
		b, pool := edgeCarrier(t, []string{"e1", "e2"}, snis("a", "b", "c"))
		pool.markSuspect("sni", "a", "tun-probe")
		pool.markSuspect("sni", "b", "tun-probe")
	...
	t.Run("the ip axis is unchanged", func(t *testing.T) {
		b, pool := edgeCarrier(t, []string{"e1", "e2", "e3"}, snis("a"))
		pool.markSuspect("ip", "e1", "tun-probe")
		pool.markSuspect("ip", "e2", "tun-probe")
		got := poolLines(t, b.st.path)
	...
	t.Run("the two axes keep separate books", func(t *testing.T) {
		b, pool := edgeCarrier(t, []string{"e1", "e2"}, snis("a", "b"))
		pool.markSuspect("ip", "e1", "tun-probe")
		pool.markSuspect("sni", "a", "tun-probe")
		got := poolLines(t, b.st.path)
		if len(got) != 2 || !strings.HasPrefix(got[0], "degraded ip:1/2") ||
			!strings.HasPrefix(got[1], "degraded sni:1/2") {
			t.Fatalf("pool events = %v, want one degraded line per axis. One watch shared by both axes "+
				"would swallow the second, and the operator would be told only half of what stopped", got)
		}
	})
}
